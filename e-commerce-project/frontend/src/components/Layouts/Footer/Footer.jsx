...
const footerLinks = [
  {
    ...
    links: [
      ...
      { name: "Stories", redirect: "#" },
      { name: "Press", redirect: "#" },
      { name: "Wholesale", redirect: "#" },
      ...
      {!adminRoute && (
        <>
            ...
            <div className="w-full sm:w-7/12 flex flex-col sm:flex-row">
              {footerLinks.map((el, i) => (
                <div className="w-full sm:w-1/5 flex flex-col gap-2 my-3 sm:my-6 ml-5" key={i}>
                  ...
                  {el.links.map((item, i) => (
                    <a href={item.redirect} target="_blank" rel="noreferrer" className="hover:underline" key={i}>
                      {item.name}
                    </a>
                  ))}
                </div>
          ...
          <div className="px-16 py-6 w-full bg-primary-darkBlue hidden sm:flex justify-between items-center text-sm text-white">
            ...
            <span>&copy; 2007-{new Date().getFullYear()} OurSite.com</span>
